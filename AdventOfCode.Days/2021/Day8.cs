...
public class Day8 : AdventDay<SignalPattern[], int, int>
{
    ...
    {
        return input.Sum(pattern =>
            pattern.Output.Count(o => new[] { 2, 3, 4, 7 }.Contains(o.Length))
        );
    }
        ...
        foreach (var (uniquePatterns, output) in input)
        {
            ...
            result += int.Parse(
                string.Join(
                    string.Empty,
                    output
                        .Select(x => string.Join(string.Empty, x.OrderBy(c => c)))
                        .Select(o => patternMap.First(x => x.Value == o).Key)
                )
            );
        }
    ...
    private static Dictionary<char, string> TransformToPatternMap(string[] uniquePatterns)
    {
        ...
        var sixSegmentsPatterns = uniquePatterns
            .Where(x => x.Length is 6 && x != patternMap['9'])
            .ToArray();

        patternMap['5'] = uniquePatterns.First(x =>
            x.Length is 5 && sixSegmentsPatterns.Any(ss => x.All(ss.Contains))
        );
        patternMap['6'] = sixSegmentsPatterns.First(x => patternMap['5'].All(x.Contains));
        patternMap['0'] = sixSegmentsPatterns.First(x =>
            x != patternMap['6'] && x != patternMap['9']
        );
        patternMap['3'] = uniquePatterns.First(x =>
            x.Length is 5 && patternMap['7'].All(x.Contains)
        );
        patternMap['2'] = uniquePatterns.First(x =>
            x.Length is 5 && x != patternMap['5'] && x != patternMap['3']
        );
        return patternMap;
    }
}
