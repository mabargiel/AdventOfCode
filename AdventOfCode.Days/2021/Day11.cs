...
namespace AdventOfCode.Days._2021
{
    public class Day11 : AdventDay<int[,], int, int>
    {
        ...
        public override int Part1(int[,] input)
        {
            ...
            for (var i = 0; i < 100; i++)
            {
                input = FlashOctopuses(IncreaseEnergyLevel(input));
        ...
        private static int[,] FlashOctopuses(int[,] octopuses)
        {
            var result = (int[,]) octopuses.Clone();
            var theyWillFLash = result.Where(o => o > 9).ToArray();
            do
            {
                ...
                theyWillFLash = result.Where(o => o > 9).ToArray();
            } while (theyWillFLash.Any());
