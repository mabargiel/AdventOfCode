...
public class Day4 : IAdventDay<int, int>
{
    ...
    public int Part2()
    {
        var validPasswords = PreValidate()
            .Where(passport =>
                ...
                && (
                    (
                        ...
                        && int.Parse(passport["hgt"].Substring(0, passport["hgt"].Length - 2))
                            is >= 150
                                and <= 193
                    )
                    || (
                ...
                && new[] { "amb", "blu", "brn", "gry", "grn", "hzl", "oth" }.Contains(
                    passport["ecl"]
                )
    ...
    {
        var validPassports = _batchFiles.Where(it =>
            it.Keys.Count == 8 || (it.Keys.Count == 7 && !it.ContainsKey("cid"))
        );
