        ...
        _boardComputer.OnOutput += output =>
        {
            if (_moveMode)
            {
                ...
                _facingDirection = output == 0
                    ? linkedListNode?.Previous?.Value ?? _allDirections.Last?.Value ?? throw new ArgumentException()
                    : linkedListNode?.Next?.Value ?? _allDirections.First?.Value ?? throw new ArgumentException();
